...
function buildUrlFromShort(short) {
    const m = short.match(/NPC(\d+)([A-Za-z])/i);
    if (!m)
        throw new Error('短縮形式が不正です。例: NPC004B');
...
function extractPythonFromHtml(html) {
    ...
    if (byId)
        return { code: byId, seeds: extractSeedsFromHtml($) };
    // 2) var code = `...` pattern
    const m = html.match(/var\s+code\s*=\s*`([\s\S]*?)`;/);
    if (m)
        return { code: m[1], seeds: extractSeedsFromHtml($) };
    // 3) first pre that looks like python
    const pres = $('pre');
    for (let i = 0; i < pres.length; i++) {
        const text = $(pres[i]).text();
        if (/^\s*(def |class |import |from )/m.test(text))
            return { code: text.trim(), seeds: extractSeedsFromHtml($) };
    }
    throw new Error('Python generator を HTML から見つけられませんでした。');
}
/* ----------------------- workspace file helpers --------------------------- */
async function saveGenToWorkspace(code, filename) {
...
function replaceOrAppendSeed(code, seed) {
...
function runChildWithTimeout(p, timeoutMs, onKill) {
    const to = setTimeout(() => {
        try {
            p.kill('SIGKILL');
        }
...
async function runGeneratorAndMain(genPath, mainPath, seed, timeoutMs, maxOutputBytes, outputDir) {
    ...
    if (seed)
        genCode = replaceOrAppendSeed(genCode, seed);
    ...
    if (mainExt === '.cpp') {
        ...
        await new Promise((resolve, reject) => {
            const compile = (0, child_process_1.spawn)('g++', ['-O2', '-std=c++17', mainPath, '-o', exe]);
            ...
            compile.on('close', (code) => {
                ...
                if (code === 0)
                    ...
                    reject(new Error('compile failed: ' + serr));
            });
    ...
    const genProc = (0, child_process_1.spawn)(pythonPath, [tempGen], { stdio: ['ignore', 'pipe', 'pipe'] });
    let mainProc;
    if (path.extname(execPath) === '.py') {
        mainProc = (0, child_process_1.spawn)(pythonPath, [execPath], { stdio: ['pipe', 'pipe', 'pipe'] });
    }
    else {
        mainProc = (0, child_process_1.spawn)(execPath, [], { stdio: ['pipe', 'pipe', 'pipe'] });
    }
    // timeouts & kill logic
    let killed = false;
    const killBoth = () => { try {
        genProc.kill('SIGKILL');
    }
    catch { } try {
    ...
    genProc.stdout.on('data', (chunk) => {
        totalForwarded += chunk.length;
        if (totalForwarded > maxOutputBytes) {
            killBoth();
            return;
        }
        try {
            mainProc.stdin?.write(chunk);
        }
        catch { }
    });
    genProc.on('close', () => { try {
        mainProc.stdin?.end();
    }
    catch { } });
    ...
    await new Promise((resolve, reject) => {
        mainProc.on('close', (code) => {
            ...
            clearMainTo();
            if (killed)
                return reject(new Error('Killed due to timeout or output limit'));
            if (exceeded)
                return reject(new Error('Output limit exceeded'));
            if (code !== 0)
                return reject(new Error('main exited with code ' + code + '\n' + stderrAll));
            resolve();
        });
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('npc.fetchGen', async () => {
        try {
            ...
            const genFilename = vscode.workspace.getConfiguration('npc').get('genFilename', 'gen.py');
        ...
        try {
            ...
            if (!seeds || seeds.length === 0) {
                ...
                if (seedInput && seedInput.trim() !== '')
                    seeds = seedInput.split(/[\s,]+/).filter(s => /^\d+$/.test(s));
            }
            if (!seeds || seeds.length === 0)
                seeds = [null];
            const timeoutMs = vscode.workspace.getConfiguration('npc').get('timeoutMs', 30000);
            const maxOutputBytes = vscode.workspace.getConfiguration('npc').get('maxOutputBytes', 10000000);
            const mainFilename = vscode.workspace.getConfiguration('npc').get('mainFilename', 'main.py');
            ...
            for (const f of files.sort())
                combined += fs.readFileSync(path.join(outputDir, f), 'utf8') + '\n';
