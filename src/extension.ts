...
async function runSingleSeed(seed: string, cfg: any, root: string, outputDir: string, outputChannel: vscode.OutputChannel, timeoutMsOverride?: number): Promise<{ seed: string; ok: boolean; exitCode?: number; err?: string; stdout?: string; mainTimeMs?: number; realTimeMs?: number }> {
  ...
  try {
    ...
    if (code !== 0) {
      ...
      return { seed, ok: false, exitCode: code ?? undefined, err: msg, stdout: outString, mainTimeMs: cpuMs, realTimeMs: realMs };
    }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const runJudgeCommand = vscode.commands.registerCommand("npc.runJudge", async () => {
    try {
      ...
      if (!(await fileExists(seedsFile))) {
        vscode.window.showErrorMessage("seeds.txt not found in workspace root. Create seeds.txt with one seed per line.");
        return;
      }
      const raw = (await readFileAsync(seedsFile, "utf8")).toString();
      ...
      // build scorer if present and build command given
      if ((cfg.scorerBuildCommand && String(cfg.scorerBuildCommand).trim().length > 0) && (await fileExists(path.join(root, String(cfg.scorerFilename))))) {
        outputChannel.appendLine(`Running scorer build: ${cfg.scorerBuildCommand}`);
        try {
          let buildCommandStr = String(cfg.scorerBuildCommand).replace(/\{out\}/g, `${cfg.scorerExec}`);
          ...
          while (true) {
            ...
            try {
              ...
              for (let attempt = 1; attempt <= attempts; attempt++) {
                ...
                outputChannel.appendLine(`Seed ${s}: attempt ${attempt} result: ok=${r.ok} measured=${measuredStr} err=${r.err ?? ""}`);
              ...
              if (acceptedRun) {
                ...
                // no accepted run within retries -> mark TLE or use lastRun as reference
                if (lastRun) {
                  // If lastRun.err is RE or indicates RE, preserve that in err but set status to RE in summary writer
                  ...
                  finalResult = {
                    ...
                    mainTimeMs: lastRun.mainTimeMs,
                    realTimeMs: lastRun.realTimeMs,
                    scorer_ok: null,
                    scorer_exitCode: null,
                    scorer_err: null,
                    scorer_stdout: null,
                  ...
                  outputChannel.appendLine(`Seed ${s}: all attempts exceeded time limit -> marked TLE/ERR (see err)`);
                } else {
                  finalResult = {
                    seed: s,
                    main_ok: false,
      ...
      // write judge summary
      try {
        writeSummaryWithStats(outdir, seeds, results, threads, root, outputChannel, "NPC Judge Summary");
        try {
          const doc = await vscode.workspace.openTextDocument(path.join(outdir, "summary.md"));
          await vscode.window.showTextDocument(doc, { preview: true });
        } catch {}
  ...
  context.subscriptions.push(fetchCommand, runCommand, runHeuristicCommand, runJudgeCommand);
}
